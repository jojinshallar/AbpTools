...
namespace AbpProjectArchitectureGenerator.Core.Cores.TemplateDatas
{
    public class EmbbedArchGenerator
    {
        ...
        const string SpecialFolderFlag = "__";
        ...
        {
            EntityTemplateData data = new(projectName, entityName, idtype, useridtype, includeLocalUsing);
        ...
        static FileInfo GetOutputFileInfo(string embbedFilePath, DirectoryInfo outputFolder, string entityname, Assembly assembly)
        {
            ...
            string outputFile = Path.Combine(outputFolder.FullName, relativeFolderFilePath.Replace(LiquidExtend, ".cs")).Replace(EntityFlag, entityname).Replace(EmbbedEntityFlag, entityname).Replace(SpecialFolderFlag,".");
